...
final class Creative_Repository {
	 ...
	 * Constructor.
	 *
	 * `unpublished_for_campaign()` is the only thing here that needs it: whether
	 * a creative is waiting for review is answered by whether it has an
	 * attachment, and that question moved out with the rest of the cluster.
	 *
	 * Defaulted rather than required, because the collaborator is stateless and
	 * holds nothing — so the five places that build a `Creative_Repository` did
	 * not have to grow a container between them and it. Still a parameter, so a
	 * test can substitute one.
	 *
	 * @param Creative_Attachment_Repository $attachments Media Library reader.
	 */
	public function __construct(
		private readonly Creative_Attachment_Repository $attachments = new Creative_Attachment_Repository()
	) {
	}
	...
	public function unpublished_for_campaign( int $campaign_id ): array {
		...
		foreach ( $this->ids_for_campaign( $campaign_id ) as $creative_id ) {
			...
			if ( $this->is_active( $creative_id ) && ! $this->attachments->has_attachment( $creative_id ) ) {
