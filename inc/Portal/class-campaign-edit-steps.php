 ...
 *
 * Three steps at most, as creation has: what the campaign is and when it runs
 * together, then its links, then review. The step keys are the ones
 * `Campaign_Actions::CHANGE_STEPS` accepts, so a posted `next_step` still
...
final class Campaign_Edit_Steps {
	...
	/**
	 * The steps for the fields a site lets advertisers change, in order.
	 *
	 * @param array<int, string> $fields Live-edit fields the site allows.
	 * @return array<string, string> Step key → label.
	 */
	public static function for_fields( array $fields ): array {
		$details = self::has_details( $fields );
		$dates   = self::has_dates( $fields );
		$steps   = array();

		if ( $details || $dates ) {
			$steps[ self::DETAILS ] = match ( true ) {
				$details && $dates => __( 'Details & dates', 'aggressive-ads' ),
				$details           => __( 'Details', 'aggressive-ads' ),
				default            => __( 'Dates', 'aggressive-ads' ),
			};
		}

		if ( in_array( 'click_urls', $fields, true ) ) {
			$steps[ self::DESTINATION ] = __( 'Links', 'aggressive-ads' );
		}
	...
	/**
	 * Whether the name, placements or notes may change.
	 *
	 * @param array<int, string> $fields Live-edit fields the site allows.
	 * @return bool
	 */
	public static function has_details( array $fields ): bool {
		return array() !== array_intersect( array( 'title', 'advertiser_notes', 'placement_ids' ), $fields );
	}
