 ...
 * for as long as the notice is on screen and nowhere else: not in the list, not
 * in a refetch, not in the audit row. That is a property of the server — no read
 * path back to it exists — and this file must not become the place that
 * quietly re-adds one, so the token is never written anywhere but state.
 *
 ...
 * and every one of those questions is a sort or a filter over a column rather
 * than a scan down an unordered table.
 */
...
import type { Credential, Strings } from './types';
...
export function Credentials( { path, advertisers, i18n }: Props ) {
	...
	const copy = async () => {
		try {
			...
			// A browser without clipboard access, or a page served over
			// http — the secret is on screen to be selected either way, so
			// this is a button that quietly does not confirm rather than an
			// error worth interrupting an issue for.
			setCopied( false );
		}
	};
	...
	const actions: Action< Credential >[] = useMemo(
		...
		// `revoke` closes over nothing that changes between renders except the
		// setters, which React keeps stable, so rebuilding the action on every
		// render would only make DataViews re-render the menu.
	...
	return (
		<Card style={ { marginTop: '16px' } }>
			...
			<CardBody>
				...
				<h3>{ i18n.newCredential }</h3>

				{ 0 === advertisers.length ? (
					<p>{ i18n.noAdvertisers }</p>
				) : (
					<>
						<SelectControl
				...
				<DataViews< Credential >
					...
					empty={ <p>{ i18n.credentialsNone }</p> }
				/>
			</CardBody>
		</Card>
	);
