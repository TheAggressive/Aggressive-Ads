...
const projectMetrics = [...xml.matchAll(/<metrics\b([^>]*)\/>/g)].at(-1)?.[1];

if (!projectMetrics) {
	throw new Error(`ci:coverage: no project metrics found in ${reportPath}`);
}
...
if (statements === 0) {
	throw new Error(`ci:coverage: ${reportPath} contains no executable statements`);
}

const percent = (coveredStatements / statements) * 100;
console.log(
	`ci:coverage: ${coveredStatements}/${statements} statements (${percent.toFixed(2)}%; minimum ${MIN_LINE_PERCENT.toFixed(2)}%)`,
);
