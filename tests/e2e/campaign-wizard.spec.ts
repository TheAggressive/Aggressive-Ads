...
test( 'advertiser completes and submits the accessible six-step wizard', async ( { page } ) => {
	...
	const portalCssLoaded = await page.evaluate( () =>
		Array.from( document.styleSheets ).some( ( sheet ) =>
			sheet.href?.includes( '/plugins/aggressive-ads/dist/styles/portal.css' )
		)
	);
	...
	await expect( page.getByRole( 'heading', { level: 2, name: 'Campaign details' } ) ).toBeFocused();
	...
	await expect( page.getByRole( 'heading', { level: 2, name: 'Choose a package' } ) ).toBeFocused();
	...
	await expect( page.getByRole( 'heading', { level: 2, name: 'Upload creative' } ) ).toBeFocused();
	...
	await expectDialogKeyboard( page, previewTrigger, 'Preview Article sidebar' );
	...
	await expect(
		page.getByRole( 'heading', { level: 2, name: 'Confirm destinations and schedule' } )
	).toBeFocused();
	...
	await expect( page.getByRole( 'heading', { level: 2, name: 'Review your campaign' } ) ).toBeFocused();
	...
	await expect( page.getByRole( 'heading', { level: 2, name: 'Submit your campaign' } ) ).toBeFocused();
	...
	await update.click();
	await expect( page.getByRole( 'dialog', { name: 'Update Article sidebar' } ) ).toBeVisible();
	...
	await expect( page.getByRole( 'dialog', { name: 'Update Article sidebar' } ) ).toBeHidden();
