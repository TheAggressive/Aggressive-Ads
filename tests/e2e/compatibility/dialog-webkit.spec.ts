...
test( 'the shared creative dialog works in WebKit', async ( { page } ) => {
	...
	await page.getByLabel( 'Campaign name' ).fill( `E2E browser campaign WebKit ${ Date.now() }` );
	...
	await upload.getByLabel( 'Destination URL' ).fill( 'https://www.example.com/webkit' );
	...
	await expectDialogKeyboard(
		page,
		page.getByRole( 'link', { name: 'Advertisement linking to example.com' } ),
