...
test( 'administrator creates a placement with a common size and a custom size', async ( { page } ) => {
	/*
	...
	await expect( page.getByRole( 'status' ).filter( { hasText: 'Placement created.' } ) ).toBeVisible();
	await expect( page.getByRole( 'heading', { name: /E2E custom slot/ } ) ).toBeVisible();
	await expect( page.getByRole( 'heading', { name: /E2E browser placement/ } ) ).toBeVisible();
	await expectAdminA11y( page );
} );
