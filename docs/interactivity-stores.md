...
| `aggr/upload` | **Shipped** | Drag/drop onto the native file input, client size/type/dimension checks, and the send itself with visible progress. The form goes by `XMLHttpRequest` (`shared/upload-send.ts`) so `upload.onprogress` can drive the card's Uploading pill, `<progress>` bar and percentage; the request follows the handler's redirect and the page then goes to where it landed, so every success notice and refusal is the server's own, rendered as before. The status region is told when a send starts and ends, never each percent. A browser without upload progress submits the ordinary way, and a failed request is never retried — the file may already be on the server — it puts the button back instead. A send can be cancelled from the card, gives up after two minutes rather than saying "Uploading" for ever, says which of the three happened (cancelled, timed out, did not finish), and while bytes are in flight the browser asks before the page is left. The post URL is read from the `action` **attribute**: WordPress forms carry a field named `action`, which shadows `form.action` |
