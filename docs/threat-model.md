...
| Decompression bomb | Size and pixel caps enforced **before** any image processing. The pixel cap is fixed (`width × height ≤ 25,000,000`); the byte cap is the placement's own, defaulting to 150 KB and never exceeding a 2 MB ceiling whatever a publisher sets. Both the enforced limit and the stored one are clamped by `Domain\Upload_Rules`, so no request body and no edited meta row can raise a placement past the ceiling |
| Content swap after review | `sha256` recorded at upload, re-verified at promotion |
| Reading stored creative off the filesystem — a served uploads directory, a backup, a `wp-content` copy, a shared host | **Encrypted at rest.** XChaCha20-Poly1305 secretstream, chunked so reads stay constant-memory and truncation is detectable. A file that is served, copied or restored without the key is an authenticated blob |
| Substituting stored creative on disk | The same AEAD. A flipped byte, a truncation, a re-ordered chunk and a file written under another key are all read failures, not different artwork |
...
**On encryption at rest:** the layer that does not depend on server configuration. The deny files this plugin writes are ignored by nginx; the UUID filename is unguessable but a backup does not have to guess; the streaming endpoint authorizes every read but a filesystem copy goes around it. Encryption is the one control that survives all four. The key lives in `AGGR_CREATIVE_KEY` or in an option — see [data-schema.md](data-schema.md) — and never in the directory it protects.

It is not a substitute for the deny rule, and the Site Health check still asks for one. It is what makes a served probe a missing layer rather than an exposure, and it is why that check reports a recommendation instead of putting a red banner on every admin page of every nginx site.

Approved creative is **not** encrypted: promotion copies it into the Media Library, which is public by definition — that is what publishing an ad means — and deletes the private original. What encryption protects is the window between upload and a reviewer's decision.
...
The route **takes no URL**. It names a campaign and reads that campaign's stored `default_click_url` — or, with `proposed=1`, the link staged in a change to that running campaign, which `Live_Edit_Rules` validated on the way into storage. `proposed` chooses *which stored link*, never a link: pointing this at an address still means first saving it to a campaign you own, through validation that already refuses credentials and non-`http(s)` schemes. The edit flow stages the link through the same form handler, nonce and rules as a form post before it checks. Three layers then apply, in `Domain\Link_Check_Rules` and `Workflow\Link_Checker`:
...
**A form posts the act it offers, not the field it writes.** The variant pause control is the clearest case: `live → cancelled` is a legal assignment transition, and `Workflow\Assignment_Editor` allows it because the routes meant to offer withdrawal need it. Had the pause button posted a `status`, a hand-made post to that action would have cancelled an assignment — terminally, since the same button could not resume what it had cancelled. It posts `intent=pause|resume`, and `Portal\Creative_Actions::process_status()` maps that to the one status the button may write; anything else is refused with `aggr_creative_status_intent_invalid` before the editor is reached. `CreativeVariantControlsTest` posts `cancelled`, `completed` and a raw status string at it and asserts the row does not move, and the guard was verified by mutating the map to forward its input and watching that test fail.
