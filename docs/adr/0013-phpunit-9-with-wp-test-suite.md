...
PHPUnit is pinned to **9.6** with `yoast/phpunit-polyfills:^4.0`, and there are **three configuration files**:
...
| `phpunit-multisite.xml.dist` | `tests/php/bootstrap-wp.php` | `multisite` | Same, with `WP_TESTS_MULTISITE` |

Separate configs because **PHPUnit allows exactly one bootstrap per configuration file**. That is the reason for the split, not preference: the unit suite must not load WordPress, and the WordPress suites must. Multisite is its own file so colliding-id tests cannot `markTestSkipped()` on the single-site lane. See [ADR-0034](0034-site-scoped-tenancy.md).
...
- Three configs mean the WordPress job runs two PHPUnit invocations (`ci:php:wp`). `ci:php` is fast and needs nothing; `ci:php:wp` needs a MySQL service. Making the fast lane genuinely fast is worth the extra files.
...
Multisite colliding-id tests cannot live in the single-site integration
config: `failOnSkipped` is true, and `factory()->blog` does not exist unless
WordPress booted as a network. `phpunit-multisite.xml.dist` uses the same
bootstrap with `WP_TESTS_MULTISITE`. See [ADR-0034](0034-site-scoped-tenancy.md).
