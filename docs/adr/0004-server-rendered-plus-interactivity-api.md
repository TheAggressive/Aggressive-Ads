...
- Known runtime hazards (from the reference implementation and the first shipped dialog): `data-wp-init` can fire twice, so any `init` binding listeners must be idempotent; exit animations need an imperative `transitionend` listener with a `setTimeout` fallback, because `transitionend` does not fire for a hidden or interrupted transition and the dialog then never closes; and nested `data-wp-class` bindings on per-instance paths such as `state.dialogs[context.dialogId].isOpen` are not reliable enough for open visibility — the dialog store opens and closes imperatively (`classList` + trigger listeners), while still keeping keyed `isOpen` in state. Details: [interactivity-stores.md](../interactivity-stores.md).
