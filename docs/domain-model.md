...
8. **A portal account belongs to exactly one organization**, enforced by `Organization_Membership::eligible_for_org()`. Staff correct a mis-assignment by removing the member and inviting them to the other organization, and the screen says so rather than offering a "move" button.
    ...
    The problem is everywhere that must name **one** organization, which today resolves it by taking the first membership and not looking further — `Campaign_Editor` when creating a campaign, `Portal\View_Data` for the top bar, `Portal\Report_Actions` for the figures, `Portal\Organization_Actions` for the tenant it writes members into. Each reads `$org_ids[0]` (or `reset()`). The list-shaped readers — the authorization test above, and the campaign list in `REST\Campaigns_Controller` — are already correct and are not the ones to worry about. Given a second membership those do not error; they silently pick one, so a campaign is filed against an arbitrary organization and a report shows an arbitrary organization's numbers. Multi-organization membership therefore means giving every one of those call sites an explicit acting organization, and giving the portal a way to choose it — a tenancy change threaded through creation, reporting and chrome, not a screen. See [roles-and-capabilities.md](roles-and-capabilities.md#ownership-is-the-organization-not-the-author). Do not add it as a convenience.
